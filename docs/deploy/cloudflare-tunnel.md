...
**Windows auto-start** (Startup folder mặc định, Task Scheduler tuỳ chọn): [windows.md](./windows.md).
...
- **Site down after reboot:** Docker Desktop và auto-start (Startup folder hoặc scheduled task) phải chạy; xem [windows.md](./windows.md) và `logs/startup-windows.log` nếu dùng Task Scheduler.
